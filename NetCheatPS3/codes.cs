    ...
    class codes
    {
        ...
        public static int ProcPreProcCode(Form1.CodeDB Codes, int cnt)
        {
            ...
            switch (type)
            {
                case '0':
                    ...
                    {
                        case 1:
                            byte[] stw0_1 = new byte[1];
                            stw0_1[0] = val[val.Length - 1];
                            Form1.apiSetMem(addr, stw0_1);
                            break;
                        case 2:
                            byte[] stw1_1 = new byte[2];
                            stw1_1[0] = val[val.Length - 2];
                            stw1_1[1] = val[val.Length - 1];
        ...
        public static void ParseCodeString(string data, ref Form1.CodeDB ret)
        {
            ...
            foreach (string s in splitList)
            {
                ...
                {
                    case '0': //8 bit write (edited to accomodate all sizes up to 32-bit)
                        if (val.IndexOf(' ') >= 0)
                            break;
                        val.Trim();
                        switch (val.Length)
                        {
                            case 2:
                                ret.CData[cnt].val = new byte[] { byte.Parse(misc.sLeft(val, val.Length), NumberStyles.HexNumber) };
                                break;
                            case 4:
                                byte[] sVal1 = BitConverter.GetBytes(int.Parse(misc.ReverseE(misc.sLeft(val, 4), 4), NumberStyles.HexNumber));
                                ret.CData[cnt].val = new byte[2] { sVal1[0], sVal1[1] };
                                break;
                            case 8:
